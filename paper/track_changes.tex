...
%DIF DEL manuscript_NatureMicrobiology_submission.tex   Sun Jan 21 08:19:49 2018
%DIF ADD manuscript.tex                                 Sun Jan 21 16:55:44 2018
...
So although infection with an influenza virus strain provides long-term immunity to that exact strain\cite{fluinboardingschool1978,davies1982christ,yu2008neutralizing}, the virus's rapid antigenic evolution erodes the effectiveness of this immunity to that strain's descendants within \DIFdelbegin \DIFdel{four to seven years\mbox{%DIFAUXCMD
\cite{couch1983immunity}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{$\sim$5 years\mbox{%DIFAUXCMD
\cite{couch1983immunity,kucharski2015estimating}}%DIFAUXCMD
}\DIFaddend .
...
This explanation is supported by classic experiments showing that it is easy to select viral mutants that escape most antibodies\cite{yewdell1979antigenic,webster1980determination}, as well as by the observation that mutations that alter antigenicity arise frequently during influenza's evolution globally\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{koel2013substitutions,chambers2015identification,petrie2016antibodies,neher2016prediction} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{koel2013substitutions,chambers2015identification,petrie2016antibodies,neher2016prediction,wen2016explaining} }%DIFAUXCMD
}\DIFaddend and within individual humans with long-term infections\cite{xue2017parallel}.
...
If these efforts succeed, the epitopes of broad antibodies \DIFdelbegin \DIFdel{will be under strong }\DIFdelend \DIFaddbegin \DIFadd{could come under stronger }\DIFaddend antigenic selection in human influenza virus.
...
Other work has shown that it is possible to select antigenic mutants with broad antibodies\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{yoshida2009cross,chai2016two,anderson2017natural}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{yoshida2009cross,chai2016two,ekiert2011highly,friesen2014common,dunand2015preexisting,anderson2017natural}}%DIFAUXCMD
}\DIFaddend , demonstrating that these epitopes are not entirely refractory to change.
...
\end{equation}
\DIFaddbegin \DIFadd{Importantly, Equations~\ref{eq:fracsurvive} and \ref{eq:fracsurvive_excess} correct for effects on viral growth due to normalization by the mock-selected control, and so measure only antigenicity and not viral growth provided that the virus at least grows well enough to be present in the library.
}\DIFaddend Details of how the calculations are extended to account for sequencing errors and sampling statistics are in the \nameref{sec:methods}.
...
We also \DIFdelbegin \DIFdel{selected }\DIFdelend \DIFaddbegin \DIFadd{chose }\DIFaddend a broad antibody, S139/1, that crystallographic studies have shown binds to residues in HA's receptor-binding pocket\cite{lee2012heterosubtypic}, and which can neutralize both group 1 and group 2 HAs\cite{yoshida2009cross, lee2012heterosubtypic}.
...
We performed our experiments using the \DIFaddbegin \DIFadd{lab-adapted }\DIFaddend A/WSN/1933 (H1N1) strain of influenza\DIFdelbegin \DIFdel{, which }\DIFdelend \DIFaddbegin \DIFadd{.
This strain is derived from an early seasonal H1N1 that was extensively passaged in the lab, where it adapted to become neurotropic and trypsin independent~\mbox{%DIFAUXCMD
\cite{sun2010modifications}}%DIFAUXCMD
.
But despite these unusual properties, the virus }\DIFaddend is neutralized by \DIFdelbegin \DIFdel{all six antibodies }\DIFdelend \DIFaddbegin \DIFadd{most broad antibodies that target other H1 viruses, including those used in this study }\DIFaddend (Figure~\ref{fig:neutcurves}).
...
For instance, at concentrations that neutralize 99\% of the wildtype virus, we expect a larger fraction of a library to survive an antibody \DIFdelbegin \DIFdel{with many }\DIFdelend \DIFaddbegin \DIFadd{for which there are many HA }\DIFaddend escape mutations than an antibody with few \DIFaddbegin \DIFadd{HA }\DIFaddend escape mutations.
...
\DIFaddbegin \DIFadd{Importantly, as described previously\mbox{%DIFAUXCMD
\cite{doud2016accurate}}%DIFAUXCMD
, these virus libraries were generated by mutagenizing HA at the }\emph{\DIFadd{codon}} \DIFadd{level rather than at the nucleotide level.
...
\DIFaddend The correlations among replicates \DIFaddbegin \DIFadd{of the mutational antigenic profiling in terms of the measured fraction-surviving above average for each possible amino-acid mutations }\DIFaddend are shown in Figure~\ref{suppfig:corr}.
...
The mutations K280S, K280T, N291S, G47R (HA2), and K(-8)T are all expected to increase neutralization resistance based on the mutational antigenic profiling (Figure~\ref{fig:structures}A)\DIFdelbeginFL \DIFdelFL{. 
The mutations }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{, whereas }\DIFaddendFL K280A, M17L (HA2), P80D, and V135T are \emph{not} expected to affect neutralization \DIFdelbeginFL \DIFdelFL{based on the mutational antigenic profiling }\DIFdelendFL (Figure~\ref{suppfig:FI6v3logo}).
\DIFaddbeginFL \DIFaddFL{All neutralization curves in this panel were performed in triplicate on the same day.
This panel shows the average of the replicates; Figure~\ref{suppfig:FI6v3replicates} shows the curves for each replicate individually and performs statistical testing of whether the IC50s for mutants are significantly different than for wildtype.
}\DIFaddendFL (B), (C) In contrast to FI6v3, mutations selected by narrow antibodies have very large effects on neutralization.
...
We created viruses carrying each of these mutations (K280S, K280T, and N291S) and validated that all three modestly \DIFaddbegin \DIFadd{but significantly }\DIFaddend increased resistance to FI6v3 (Figure~\ref{fig:FI6v3neutcurves}A\DIFaddbegin \DIFadd{, Figure~\ref{suppfig:FI6v3replicates}}\DIFaddend ).
As a control, we also validated that a mutation at one of these sites (K280A) that does \emph{not} have an effect in our mutational antigenic profiling does not \DIFaddbegin \DIFadd{significantly }\DIFaddend shift the neutralization curve (Figure~\ref{fig:FI6v3neutcurves}A\DIFaddbegin \DIFadd{, Figure~\ref{suppfig:FI6v3replicates}}\DIFaddend ).

Our mutational antigenic profiling also identified several non-glycosylation-motif mutations that were selected by FI6v3.
We validated that one of these mutations, G47R in the HA2 chain, \DIFaddbegin \DIFadd{significantly }\DIFaddend increased neutralization resistance (Figure~\ref{fig:FI6v3neutcurves}A\DIFaddbegin \DIFadd{, Figure~\ref{suppfig:FI6v3replicates}}\DIFaddend )---although as predicted by the mutational antigenic profiling, the magnitude of the effect was small.
The most unexpected mutations identified in our mutational antigenic profiling were at site -8 in the signal peptide.
We tested one of these mutations, K(-8)T, and it did \DIFdelbegin \DIFdel{appear to }\DIFdelend lead to a very slight increase in neutralization resistance (Figure~\ref{fig:FI6v3neutcurves}A\DIFdelbegin \DIFdel{), although we are not confident that this effect exceeds }\DIFdelend \DIFaddbegin \DIFadd{, Figure~\ref{suppfig:FI6v3replicates})---although despite the significance testing in Figure~\ref{suppfig:FI6v3replicates}, we remain circumspect about the magnitude of this effect relative to }\DIFaddend the noise in our neutralization assays.
As controls, we also tested three mutations (P80D and V135T, which are escape mutations for H17-L7 and H17-L19, and M17L in HA2) that did \emph{not} have substantial effects in the mutational antigenic profiling, and confirmed that none of them \DIFaddbegin \DIFadd{significantly }\DIFaddend affected neutralization resistance \DIFdelbegin \DIFdel{within the sensitivity of our assays}\DIFdelend \DIFaddbegin \DIFadd{(Figure~\ref{fig:FI6v3neutcurves}A, Figure~\ref{suppfig:FI6v3replicates})}\DIFaddend .
...
\section*{DISCUSSION}
We have quantified how all single amino-acid mutations to \DIFaddbegin \DIFadd{an H1 }\DIFaddend influenza virus HA affect neutralization by a collection of broad and narrow antibodies.
...
There are no such single mutations for the \DIFaddbegin \DIFadd{H1 }\DIFaddend HA and broad anti-stalk antibodies tested here, since no mutations shift the neutralization curve enough to enable survival at antibody concentrations that fully neutralize wildtype.
...
\DIFaddbegin \DIFadd{Interestingly, most previous studies~\mbox{%DIFAUXCMD
\cite{chai2016two,ekiert2011highly,friesen2014common,dunand2015preexisting} }%DIFAUXCMD
that have reported selecting single mutations with large effects ($\gg$10-fold) on neutralization by anti-stalk antibodies have used group 2 (e.g., H3 or H7) HAs rather than group 1 HAs like the one used in our work.
}\DIFaddend When interpreting the magnitude of these effects, it is important to note that our experiments only measure how mutations affect neutralization, and not how they affect Fc-mediated functions that are responsible for much of the \textit{in vivo} protection afforded by anti-stalk antibodies\cite{dilillo2014broadly,dilillo2016broadly}.
...
All neutralization curves \DIFaddbegin \DIFadd{in Figure~\ref{fig:FI6v3neutcurves}A }\DIFaddend represent the mean and standard deviation of three measurements\DIFdelbegin \DIFdel{. 
}\DIFdelend \DIFaddbegin \DIFadd{, with the individual replicates shown in Figure~\ref{suppfig:FI6v3replicates}.
...
\DIFaddend \subsection*{H3 sequence numbering}
...
The deep sequencing data were analyzed using version \DIFdelbegin \DIFdel{2.1.0 }\DIFdelend \DIFaddbegin \DIFadd{2.2.1 }\DIFaddend of the \texttt{dms\_tools2} software package\cite{bloom2015software}, which is available at \url{http://jbloomlab.github.io/dms_tools2}.
...
In this paper, Figures~\ref{fig:avgfracsurvive} and \ref{suppfig:maxfracsurvive} show the median of excess fraction surviving taken across all biological and technical replicates at a given antibody concentration \DIFaddbegin \DIFadd{(Equation~\ref{eq:fracsurvive_excess})}\DIFaddend .
The subsequent logo plots show the medians of these values taken across all concentrations for each antibody.
The numerical values plotted in these logo plots are in File~\DIFaddbegin \DIFadd{\ref{suppfile:fracsurvive_excess}.
The fraction surviving values }\emph{\DIFadd{not}} \DIFadd{adjusted to be in excess of the library average (Equation~\ref{eq:fracsurvive}) are in File~}\DIFaddend \ref{suppfile:fracsurvive}.
...
{\bf \DIFadd{Replicates of the FI6v3 neutralization curves in Figure~\ref{fig:FI6v3neutcurves}A.}}
\DIFadd{The neutralization assays were performed in triplicate for all nine mutants and wildtype.
Figure~\ref{fig:FI6v3neutcurves}A shows the }\emph{\DIFadd{average}} \DIFadd{of those replicates.
(A) The neutralization data for each replicate shown individually, with IC50 values fit using a four-parameter logistic curve with the top value constrained to one (see }\url{https://jbloomlab.github.io/dms_tools2/dms_tools2.neutcurve.html} \DIFadd{for the code used for the fitting.)
...
\begin{suppfile}
\caption{\DIFdelbegin %DIFDELCMD < \label{suppfile:fracsurvive}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{suppfile:fracsurvive_excess}
...
\DIFaddbegin \DIFadd{See Equation~\ref{eq:fracsurvive_excess}.
}\DIFaddend }
\DIFaddbegin \end{suppfile}

\begin{suppfile}
\caption{\label{suppfile:fracsurvive}
{\bf \DIFadd{The fraction surviving for each mutation for each antibody.}}
\DIFadd{This file differs from File~\ref{suppfile:fracsurvive_excess} only in that the values are }\emph{\DIFadd{not}} \DIFadd{adjusted to be in excess of the library average (e.g., they are from Equation~\ref{eq:fracsurvive} rather than Equation~\ref{eq:fracsurvive_excess}).
}}
